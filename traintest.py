...
# TensorFlow wizardry: Don't pre-allocate memory; allocate as-needed
...
if use_dataloader == True:
    ...
    validation_generator = cu.DataGenerator(data_loc, val_IDs, 
        ...
        y_train = y_train[:,:,:,1:] # We are going to drop the first channel of y because we don't need background info anymore
        masks/=255
        masks = np.concatenate((masks[:,:,:,np.newaxis],masks[:,:,:,np.newaxis],), axis=3)
...
metrics = [cu.iou_loss, cu.build_iou_for(label=0), cu.build_iou_for(label=1)] 
...
if use_dataloader == True:
    ...
    #sample_weight = cu.get_image_weights(y_train, masks)
    sample_weight = None
...
#y_pred = np.argmax(model.predict(x=[X_train,masks]),axis=3)+masks[:,:,:,0]
#im=7;plt.subplot(1,2,1);plt.imshow(y_train[200+im,:,:,0]+y_train[200+im,:,:,1]*2);plt.subplot(1,2,2);plt.imshow(y_pred[im,:,:]+masks[200+im,:,:,0]);